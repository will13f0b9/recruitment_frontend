  ...
  <v-app class="about">
    <v-container style="z-index: 2">
        ...
        <v-card-actions>
          ...
          <v-card class="mx-auto">
            <v-card-text>
              ...
              <div>Alteração de dados da vaga</div>
      ...
      <v-dialog v-model="dialogEdit" persistent max-width="800">
        <v-card>
          <v-card-title class="headline green--text"
            >{{action != -1 ? 'Editar questão do exame' : 'Cadastrar novas questões ao exame'}}</v-card-title
          >
          <v-card-text>
...
export default {
  ...
  methods: {
    ...
    async saveJobChanges() {
      ...
      this.mainControll.globalLoading = true;
      await job
        .changeJobData(this.$router.currentRoute.params.id, this.job)
        .then((resp) => {
          this.mainControll.globalLoading = false;
          if (resp.status == 200) {
            this.successSnackBar("Dados da vaga atualizado com sucesso!");
          } else {
            this.showInvalidSnackBar(
              err.data.message ?? "Não foi possível atualizar dados da vaga!"
            );
          }
        })
        .catch((err) => {
          ...
          this.showInvalidSnackBar(
            err.response.data.message ??
              "Não foi possível atualizar dados da vaga!"
          );
        });
    },
    removeItem(i) {
